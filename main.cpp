...
DeclareComponent_TEST(Visible);
DefineComponent_TEST(Visible);

DeclareComponent_TEST(Hittable);
DefineComponent_TEST(Hittable);

DeclareComponent_TEST(Invisible);
DefineComponent_TEST(Invisible);

int main()
{
	std::this_thread::sleep_for(std::chrono::seconds(5));
	{
		using namespace sy::test;
		...
		for (size_t count = 0; count < 100000; ++count)
		{
			...
			vRef.A = count + 0xffffff;
			vRef.B = (count + 0xf0f0f0);
		...
		std::cout << "ChunkPool - Generation: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << " ms" << std::endl;
		...
		std::cout << "ChunkPool - Validation: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << " ms" << std::endl;
	}
	{
		using namespace sy;
		auto& componentArchive = ComponentArchive::Get();
